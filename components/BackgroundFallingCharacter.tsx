...
function pickBiasedX(width: number, gutterPct: number, sidesWeight: number) {
  const gutter = Math.max(
    0,
    Math.min(Math.floor(width * gutterPct), Math.floor(width / 2))
  );
...
export function BackgroundFallingCharacters({
  ...
  const pool = useMemo(() => {
    ...
    return items.length
      ? items
      : SCRIPTS.latin.map((ch) => ({ script: "latin" as ScriptKey, char: ch }));
  }, [scriptWeights]);
    ...
    >
      ...
      {showFloor && (
        <div
          ...
          style={{
            boxShadow:
              "0 0 24px rgba(255,255,255,0.04), 0 -6px 12px rgba(255,255,255,0.025) inset",
          }}
        />
      )}
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (g.bottom >= f.top) {
        ...
        setCollision({
          x: g.left - parent.left + g.width / 2,
          y: g.bottom - parent.top,
        });
        setDetected(true);
...
const Explosion = ({ ...props }: React.HTMLProps<HTMLDivElement>) => {
  ...
  return (
    <div
      {...props}
      className={cn(
        "absolute z-20 h-2 w-2 pointer-events-none",
        props.className
      )}
    >
      <motion.div
