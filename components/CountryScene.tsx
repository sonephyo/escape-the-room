...
export function CountryScene({ country }: { country: CountryKey }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      setPlayer((prev) => {
        ...
        if (x === prev.x && y === prev.y && prev.moving === moving && prev.dir === dir) return prev;
  ...
  useEffect(() => {
    ...
    if (spot && activeSpot !== spot.id) { setActiveSpot(spot.id); setChatOpen(true); }
    ...
    <div className="w-full h-screen bg-black overflow-hidden relative flex items-center justify-center p-4">
      ...
      <div className="relative border-4 border-gray-600 shadow-2xl bg-black overflow-hidden" style={{ width: VIEW_W, height: VIEW_H }}>
        ...
        >
          {/* Hotspots */}
          {hotspots.map(h => (
            <div key={h.id} className="absolute pointer-events-none" style={{ left: h.x - h.radius, top: h.y - h.radius, width: h.radius * 2, height: h.radius * 2 }}>
      ...
      <AkoolChatModal
        ...
        agent={{ server: { id: activeSpot ?? 'none', name: 'Guide', description: `Practice in ${cfg.name}` } as any,
                 buildingName: activeSpot ? hotspots.find(h => h.id === activeSpot)?.label : undefined }}
      />
    </div>
  );
}
