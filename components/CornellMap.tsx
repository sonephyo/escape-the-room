...
import { Character as CharacterType, Position, Building as BuildingType } from '../types';
...
function isNearBuilding(px: number, py: number, b: BuildingType, pad = NEAR_PAD): boolean {
...
export const CornellMap: React.FC<CornellMapProps> = ({ n, characters }) => {
  ...
  const agents: AgentOnMap[] = useMemo(() => {
    ...
    const remaining = chosen.filter(sc => !placed.some(p => p.server.id === sc.id));
    let idx = 0;
...
}, [character.position.x, character.position.y, agents, solved, snoozed, activeAgentId]);
    ...
    <div className="w-full h-screen bg-black overflow-hidden relative flex items-center justify-center">
      ...
      >
        ...
        <div className="absolute top-4 right-4 z-30">
          ...
          >
            Agents: {n} • Solved: {Object.values(solved).filter(Boolean).length}/{agents.length}
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black text-white px-4 py-2 border-2 border-white">
        <p className="text-center" style={{ fontFamily: 'monospace', fontSize: '12px', imageRendering: 'pixelated' }}>
          Walk near a building with a marker to meet an agent • WASD / Arrow Keys to move
        </p>
      </div>
