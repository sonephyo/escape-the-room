...
export function CountrySelect({
  ...
  return (
    <div
      className="
        relative w-full max-w-6xl mx-auto p-4 sm:p-6 text-[#fbd3d3]
        rounded-[18px] border-4 border-[#ff2a2a]
        bg-[#120a0a] shadow-[0_0_0_4px_#3b0707_inset,0_0_60px_#ff2a2a40]
        font-['VT323','Press_Start_2P',ui-monospace,monospace] tracking-tight
        before:pointer-events-none before:absolute before:inset-0
        before:bg-[repeating-linear-gradient(0deg,rgba(255,255,255,0.06)_0px,rgba(255,255,255,0.06)_1px,transparent_2px,transparent_3px)]
        before:opacity-20
        after:pointer-events-none after:absolute after:inset-0 after:bg-[radial-gradient(ellipse_at_center,transparent_40%,#000_100%)]
      ...
      style={{ imageRendering: "pixelated" }}
    >
      {/* HUD Top Bar — matches StartScreen styling */}
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-5xl leading-none text-[#ff2a2a] [text-shadow:0_0_10px_#ff2a2a80,0_0_30px_#ff2a2a50] uppercase">
          ► Choose your destination
        </h1>
        <div className="hidden sm:flex items-center gap-2 text-1xl text-[#ff9d9d]">
          <span className="px-2 py-1 border-2 border-[#ff2a2a] bg-[#1a0b0b]">
            ←
          </span>
          <span className="px-2 py-1 border-2 border-[#ff2a2a] bg-[#1a0b0b]">
            →
      ...
      <div className="relative">
        {/* Neon Grid Frame */}
        <div className="pointer-events-none absolute -inset-2 -z-10 rounded-[22px] border-4 border-[#ff2a2a66]" />
        <div
          className="
            absolute -inset-1 -z-20 rounded-[22px] opacity-30
            bg-[radial-gradient(circle_at_50%_0%,#ff2a2a22,transparent_60%)]
          "
        />
        ...
        <button
          ...
          className="
            group absolute left-0 top-1/2 -translate-y-1/2 z-20
            px-3 py-3 border-4 border-[#ff2a2a] bg-[#1a0b0b]
            text-[#ff2a2a] hover:bg-[#2a0b0b] active:translate-y-[1px]
            shadow-[4px_4px_0_#690a0a] transition
          "
          aria-label="Previous"
        >
          <span className="block translate-x-[-1px] group-hover:translate-x-[-2px] transition">
        ...
        {/* Card (single, swipeable) */}
        <div className="mx-12 sm:mx-16 md:mx-24">
          <div className="relative aspect-[5/6] sm:aspect-[16/9] max-h-[68vh] overflow-hidden rounded-[12px] border-4 border-[#ff2a2a] bg-[#120a0a]">
            <AnimatePresence custom={direction} mode="popLayout">
              {(() => {
                const key = current;
                const cfg = COUNTRIES[key];
                return (
                  ...
                  >
                    ...
                    />
                    {/* Scanlines + vignette */}
                    <div className="pointer-events-none absolute inset-0 mix-blend-multiply opacity-70 bg-[repeating-linear-gradient(#00000022_0px,#00000022_2px,transparent_3px,transparent_4px)] animate-scanlines" />
                    <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_60%,#000_100%)] opacity-80" />
                    {/* Bezel */}
                    <div className="pointer-events-none absolute inset-0 border-4 border-[#ff2a2a] rounded-[8px] opacity-60" />

                    {/* HUD header */}
                    <div className="absolute left-3 top-3 flex items-center gap-2">
                      <span className="h-3 w-3 bg-[#ff2a2a] shadow-[0_0_12px_#ff2a2a] border-2 border-[#3b0707]" />
                      <span className="px-2 py-1 border-2 border-[#ff2a2a] bg-[#1a0b0b] text-[10px] text-[#ffb1b1]">
                        {String(idx + 1).padStart(2, "0")}/
                        {String(total).padStart(2, "0")}
                      </span>
                    </div>
                    ...
                    {/* Footer Card Info */}
                    <div className="absolute bottom-0 inset-x-0 p-3 sm:p-4">
                      <div className="rounded-[8px] border-4 border-[#ff2a2a] bg-[#120a0a]/90 p-3 sm:p-4 shadow-[4px_4px_0_#690a0a]">
                        <div className="text-4xl  text-[#ff2a2a] uppercase [text-shadow:0_0_8px_#ff2a2a80]">
                          {cfg.name}
                        </div>
                        <div className="mt-2 text-2xl text-[#ff9d9d]">
                          LANGUAGE:{" "}
                          <span className="text-[#fbd3d3]">{cfg.locale}</span>
                        </div>
                        <div className="mt-3 flex items-center gap-2 text-1xl text-[#ff9d9d]">
                          <span className="px-2 py-1 border-2 border-[#ff2a2a] bg-[#1a0b0b]">
                            SWIPE
                          </span>
                          <span className="px-2 py-1 border-2 border-[#ff2a2a] bg-[#1a0b0b]">
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center gap-4">
        <button
          onClick={() => onChoose(current)}
          className="
            group inline-flex items-center justify-center gap-2 px-5 py-3
            text-[#2a0b0b]
            rounded-[10px]
            border-4 border-[#ff2a2a]
            bg-[linear-gradient(180deg,#ff7a7a_0%,#ff2a2a_58%,#c61c1c_100%)]
            shadow-[inset_0_-3px_0_#b81b1b,4px_4px_0_#690a0a]
            active:translate-y-[1px] hover:brightness-95
            uppercase text-[12px]
          "
        >
          Start
          <span className="inline-block translate-y-[1px] pl-1 group-hover:translate-x-1 transition-transform">
            ►
          </span>
        </button>
        ...
        <div className="flex items-center gap-2">
          {ORDER.map((k, i) => (
            <button
              ...
              className={`
                h-3 transition-all border-2 border-[#ff2a2a] shadow-[2px_2px_0_#690a0a]
                ${
                  i === idx
                    ? "w-8 bg-[#ff2a2a]"
                    : "w-3 bg-transparent hover:bg-[#ff2a2a55]"
                }
              `}
              aria-label={`Go to ${COUNTRIES[k].name}`}
            />
      ...
      {/* Tiny helper text */}
      <p className="mt-3 text-[10px] text-[#ffb1b1] opacity-80">
        USE ← / → OR SWIPE • PRESS ENTER TO CONFIRM
      </p>
    </div>
  );
