class Character {
  constructor(scene, config) {
    ...
    this.sprite = this.scene.physics.add
      .sprite(this.spawnPoint.x, this.spawnPoint.y, this.atlas)
      .setSize(30, 40)
      .setOffset(0, 0)
      .setImmovable(true);
  ...
  createAnimations() {
    ...
    directions.forEach(direction => {
      ...
      if (!anims.exists(animKey)) {
        // Check if texture atlas has the required frames
        const textureManager = this.scene.textures;
        const texture = textureManager.get(this.atlas);
        
        if (texture && texture.frames && texture.frames[`${this.id}-${direction}-walk-0000`]) {
          anims.create({
            key: animKey,
  ...
  moveInCurrentDirection() {
    ...
    if (distanceFromSpawn > this.roamRadius) {
      ...
      // Use static texture
      this.sprite.setTexture(this.atlas);
